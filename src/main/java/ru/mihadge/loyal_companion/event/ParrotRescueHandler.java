...
public class ParrotRescueHandler {

    private static final String RESCUE_COOLDOWN = "loyal_companion_last_rescue";

    @SubscribeEvent
    public void onParrotDamage(LivingDamageEvent.Pre event) {
        ...
        if (parrot.getHealth() > event.getNewDamage()) {
            return;
        }

        if (!(parrot.getOwner() instanceof ServerPlayer owner)) {
            return;
        }

        CompoundTag data = parrot.getPersistentData();

        long currentTime = parrot.level().getGameTime();
        long lastRescue = data.getLong(RESCUE_COOLDOWN);

        if (currentTime - lastRescue < Config.RESCUE_COOLDOWN_PARROT.get() * 20 ) {
            return;
        }
        ...
        parrot.invulnerableTime = Config.EFFECTS_DURATION_PARROT.get() * 20;

        parrot.clearFire();
        parrot.setRemainingFireTicks(0);

        parrot.addEffect(new MobEffectInstance(
                MobEffects.DAMAGE_RESISTANCE,
                Config.EFFECTS_DURATION_PARROT.get() * 20,
                4,
                false,
                true
        ));
        ...
        teleportNearOwner(parrot, owner);
    }
